...
proc readerFragment { fout base name } {
     ...
     puts $fout "   \}"
     puts $fout "    // Confluent wire format v1: 1 magic byte + 4-byte big-endian"
     puts $fout "    // schema id. SAL always publishes this 5-byte prefix via"
     puts $fout "    // SchemaId::idToBytes(), so skip it directly. (The 17-byte"
     puts $fout "    // GUID prefix exists in the format but SAL never emits it.)"
     puts $fout "    payload += 5;"
     puts $fout "    len -= 5;"
     puts $fout "    auto& avro_schema = sal\[actorIdx\].avroSchema;"
...
proc writerFragment { fout base name } {
  ...
  puts $fout "     auto framing = schemaregistry::serdes::SchemaId(schemaregistry::serdes::SerdeFormat::Avro, sal\[actorIdx\].avroSchemaId).idToBytes();"
  puts $fout "     buffer.insert(buffer.end(), framing.begin(), framing.end());"
