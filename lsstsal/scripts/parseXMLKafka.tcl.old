...
#\code
#
## Documented proc \c parseXMLtoJson .
...
\"type\": \"record\", \"name\": \"[set atname]\", \"namespace\": \"lsst.sal.[set subsys]\", \"fields\": \["
      ...
      if { $tag == "EFDB_Name"} {
        ...
        if { [lsearch $AVRORESERVED [string tolower $item]] > -1 } {
           puts stdout "****************************************************************"
           puts stdout "****************************************************************"
           puts stdout "Invalid use of AVRO reserved token $item"
           puts stdout "****************************************************************"
           puts stdout "****************************************************************"
           exit 1
        }
      ...
      if { $tag == "/item" } {
         ...
         if { [lsearch $AVROTYPES $chktype] < 0 } {
           puts stdout "****************************************************************"
           puts stdout "****************************************************************"
...
\}"
   ...
   if { [info exists CMD_ALIASES($subsys)] } {
    if { $CMD_ALIASES($subsys) != "" } {
     ...
     set fout [open $SAL_WORK_DIR/avro-templates/[set subsys]_cmddef.tcl w]
   ...
   if { [info exists EVENT_ALIASES($subsys)] } {
    if { $EVENT_ALIASES($subsys) != "" } {
     ...
     set fout [open $SAL_WORK_DIR/avro-templates/[set subsys]_evtdef.tcl w]
   ...
   if { [info exists TLM_ALIASES($subsys)] } {
    if { $TLM_ALIASES($subsys) != "" } {
      ...
      set fout [open $SAL_WORK_DIR/avro-templates/[set subsys]_tlmdef.tcl w]
...
#
## Documented proc \c indexedEnumsToCPP .
# \param[in] subsys Name of CSC/SUbsystem as defined in SALSubsystems.xml
# \param[in] fout File handle of output c++ fragment file
#
#  Generate an Json const definition for a SAL XML indexed enumeration
#
...
global SYSDIC IDXENUMDONE SAL_WORK_DIR
   if { [info exists SYSDIC($subsys,IndexEnumeration)] && $IDXENUMDONE($subsys) == 0 } {
      ...
      foreach e $SYSDIC($subsys,IndexEnumeration) {
         ...
         puts $fenum " const long long indexEnumeration_[set i]=$id;"
      }
...
#
## Documented proc \c indexedEnumsToJava .
# \param[in] subsys Name of CSC/SUbsystem as defined in SALSubsystems.xml
# \param[in] fout File handle of output java fragment file
#
#  Generate an Json const definition for a SAL XML indexed enumeration
#
...
global SYSDIC IDXENUMDONE SAL_WORK_DIR
   if { [info exists SYSDIC($subsys,IndexEnumeration)] && $IDXENUMDONE($subsys) == 0 } {
      ...
      foreach e $SYSDIC($subsys,IndexEnumeration) {
         ...
         set id [lindex [split $enum :] 0]
         set i  [lindex [split $enum :] 1]
         puts $fenum " public static final int indexEnumeration_[set i]=$id;"
      }
      close $fenum
      set IDXENUMDONE($subsys) 1
   }
...
global METADATA
  ...
  set METADATA([set topic],private_efdStamp,units) "\"second\""
  set METADATA([set topic],private_kafkaStamp,units) "\"second\""
  ...
  set METADATA([set topic],private_rcvStamp,description) "\"UTC Time of instance reception\""
  set METADATA([set topic],private_efdStamp,description) "\"TAI Time of instance publication\""
  set METADATA([set topic],private_kafkaStamp,description) "\"Time of instance publication\""
...
#
## Documented proc \c enumsToCPP .
# \param[in] subsys Name of CSC/SUbsystem as defined in SALSubsystems.xml
# \param[in] alias Aliased name of Topic
# \param[in] fout File handle of output json file
#
   ...
   if { [info exists EVENT_ENUM($alias)] && [info exists EDONE($alias)] == 0} {
      ...
      foreach e $EVENT_ENUM($alias) {
          ...
          foreach id [split $cnst ,] {
              ...
              puts $fenum " const long long [set alias]_[string trim $id " "]=$i;"
              incr i 1
          }
      }
      set EDONE($alias) 1
...
global AVRORESERVED SAL_WORK_DIR SAL_DIR CMDS CMD_ALIASES EVTS EVENT_ALIASES UNITS DESC
...
global AVRORESERVED SAL_WORK_DIR SAL_DIR TLMS TLM_ALIASES UNITS DESC
  exec mkdir -p $SAL_WORK_DIR/html/[set subsys]
