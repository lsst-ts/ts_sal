...
set PRINTLEN(int) 11
set PRINTLEN(long) 11
set PRINTLEN(longlong) 19 
set PRINTLEN(short) 7
set PRINTLEN(byte) 4
set PRINTLEN(boolean) 1
set PRINTLEN(float) 11
set PRINTLEN(double) 19
set SQLTYPE(int) long
set SQLTYPE(longlong) long
set SQLTYPE(short) short
set SQLTYPE(char) octet
set SQLTYPE(byte) octet
set SQLTYPE(float) float
set SQLTYPE(double) octet
set SQLTYPE(boolean) boolean
...
proc ndarray { fid fo2 name type nd } {
global ITEMLIST PRINTLEN IDLTYPE
   switch $type {
       char   -
       Char   {
                puts $fid "  string<$nd> $name;"
       ...
       double -
       Double - 
       Int    { set maxlen [expr $PRINTLEN([string tolower $type]) * $nd]
                puts $fid "  $type $name\[$nd\];" 
                puts $fo2 "  $name varchar($maxlen),"
                set ITEMLIST "$ITEMLIST,array[set type].[set name].$nd"
              }
   }
}
...
#puts stdout "nditem $fout $fo2 $name $type $nd"
      ...
      if { $nd > 32 && $type != "string" } {
        ndarray $flog $fo2 $name $type $nd
      } else {
