...
proc insertControllers { subsys file_writer } {
    ...
    foreach alias $CMD_ALIASES($subsys) {
        ...
        puts $file_writer "  [set subsys]_command_[set alias]C flush[set alias];"
        puts $file_writer "  cmdId = mgr.acceptCommand_[set alias](&flush[set alias]);"
...
global SYSDIC
    ...
    puts $file_writer "CPPFLAGS      = \$(PICFLAGS) \$(GENFLAGS) -g \$(SAL_CPPFLAGS) -D_REENTRANT -Wall -I\".\"  -I\"\$(AVRO_INCL)\" -I../../[set subsys]/cpp/src -I\"\$(LSST_SAL_PREFIX)/include\" -I\"\$(CONDA_PREFIX)/include\" -I\"\$(LSST_SAL_PREFIX)/include/avro\" -I\"\$(CONDA_PREFIX)/include/avro\" -I\"\$(LSST_SAL_PREFIX)/include/boost\" -I\"\$(CONDA_PREFIX)/include/boost\" -I.. -I\"\$(SAL_WORK_DIR)/include\" -fpermissive -Wno-unused-variable -Wno-write-strings $keyed"
    ...
    puts $file_writer "LDFLAGS       = -L\".\" -L\"\$(LSST_SAL_PREFIX)/lib\" -L\"\$(CONDA_PREFIX)/lib\" -L\"\$(SAL_WORK_DIR)/lib\""
    ...
    puts $file_writer "# libschemaregistry replaces libserdes. Its vcpkg-built static deps"
    puts $file_writer "# (abseil, cpr, curl, ssl, compression) reference each other's symbols,"
    puts $file_writer "# so they are linked inside a --start-group that re-scans them until the"
    puts $file_writer "# cross-references resolve, making their order irrelevant. Override"
    puts $file_writer "# LIBSCHEMAREGISTRY_VCPKG_LIB until the build helper copies the deps"
    puts $file_writer "# into the SAL prefix."
    puts $file_writer "LIBSCHEMAREGISTRY_VCPKG_LIB ?= \$(LSST_SAL_PREFIX)/lib"
    puts $file_writer "LIBSCHEMAREGISTRY_LDLIBS = -Wl,--start-group \$(LSST_SAL_PREFIX)/lib/libschemaregistry.a \$(wildcard \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libabsl_*.a) \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libcpr.a \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libcurl.a \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libz.a \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libbz2.a \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/liblzma.a \$(LIBSCHEMAREGISTRY_VCPKG_LIB)/libzstd.a -Wl,--end-group"
    puts $file_writer "LDLIBS        = -ldl -lrt -lpthread -lboost_filesystem -lboost_iostreams -lboost_program_options -lboost_regex -lboost_system -lsnappy \$(LIBSCHEMAREGISTRY_LDLIBS) -lrdkafka++ -lrdkafka -lavrocpp -lsasl2 -lssl -lcrypto -llz4"
